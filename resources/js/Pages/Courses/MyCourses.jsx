...
export default function MyCourses({ enrollments = {}, allCourses = [], enrolledCourses = [], pendingCourses = [], isInternal = false }) {
    ...
    const filterCourses = (courseList) => {
        if (!debouncedSearch.trim()) return courseList;
    ...
    const filteredEnrolled = useMemo(() => filterCourses(enrolledCourses), [debouncedSearch, enrolledCourses]);
    const filteredPending = useMemo(() => filterCourses(pendingCourses), [debouncedSearch, pendingCourses]);
    const filteredAvailable = useMemo(() => filterCourses(availableCourses), [debouncedSearch, availableCourses]);

    const totalCourses = enrolledCourses.length + pendingCourses.length + availableCourses.length;
    const totalFiltered = filteredEnrolled.length + filteredPending.length + (isInternal ? filteredAvailable.length : 0);
            ...
            <div className="space-y-6">
                ...
                {debouncedSearch.trim() && (
                    <p className="text-sm text-gray-500">
                        Showing {totalFiltered} of {totalCourses} courses
                    </p>
                )}

                {/* No results at all */}
                {debouncedSearch.trim() && totalFiltered === 0 && (
                    <div className="rounded-lg bg-white p-12 text-center shadow">
                        <svg className="mx-auto h-12 w-12 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
                            <path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" />
                        </svg>
                        <h3 className="mt-2 text-sm font-medium text-gray-900">No courses found</h3>
                        <p className="mt-1 text-sm text-gray-500">No courses match &quot;{debouncedSearch}&quot;.</p>
                        <button
                            onClick={() => setSearch('')}
                ...
                {/* Pending Approval */}
                {(filteredPending.length > 0 || (!debouncedSearch.trim() && pendingCourses.length > 0)) && (
                    <div className="min-h-[100px]">
                ...
                {isInternal && (filteredAvailable.length > 0 || (!debouncedSearch.trim() && availableCourses.length > 0)) && (
                    <div className="min-h-[100px]">
